...
export const MediaCardTwitter = ({ 
  ...
  const startVideoPlayback = useCallback(() => {
    ...
    if (!vs.previewEnabled) return; // preview disabled = no hover playback
  ...
  const renderHeader = () => (
    showHeader && (
      <div 
        ...
        style={{ padding: advancedSettings.headerPadding, paddingBottom: advancedSettings.headerPadding * 0.75 }}
      >
        <div 
          ...
          style={{ width: advancedSettings.avatarSize, height: advancedSettings.avatarSize, fontSize: advancedSettings.avatarSize * 0.35 }}
        >
          MV
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-foreground" style={{ fontSize: advancedSettings.usernameFontSize }}>MediaVault</span>
            <span className="text-muted-foreground" style={{ fontSize: advancedSettings.usernameFontSize * 0.9 }}>@mediavault</span>
            <span className="text-muted-foreground" style={{ fontSize: advancedSettings.usernameFontSize * 0.9 }}>·</span>
            <span className="text-muted-foreground" style={{ fontSize: advancedSettings.usernameFontSize * 0.9 }}>{timeAgo}</span>
          </div>
          {showTitle && (
            <p className="text-foreground mt-1" style={{ fontSize: advancedSettings.titleFontSize, display: '-webkit-box', WebkitLineClamp: advancedSettings.titleLineClamp, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
              {item.name}
            </p>
          )}
        </div>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" className="opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" style={{ width: advancedSettings.actionButtonSize, height: advancedSettings.actionButtonSize }}>
              <MoreHorizontal style={{ width: advancedSettings.actionIconSize, height: advancedSettings.actionIconSize }} />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={onView}><Eye className="w-4 h-4 mr-2" />Voir</DropdownMenuItem>
            <DropdownMenuItem onClick={onDownload}><Download className="w-4 h-4 mr-2" />Télécharger</DropdownMenuItem>
            <DropdownMenuItem onClick={onDelete} className="text-destructive"><Trash2 className="w-4 h-4 mr-2" />Supprimer</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    )
  ...
  const renderMedia = () => (
    ...
    >
        ...
        <>
          ...
          {!isVideoPlaying && thumbnailUrl && (
            <img
              src={thumbnailUrl}
              alt={item.name}
              className={cn("absolute inset-0 w-full h-full object-cover transition-all duration-500", isHovered && "opacity-0")}
              loading="lazy"
            />
          )}
        </>
      ...
      {item.type === 'video' && !isVideoPlaying && (
        <div className={cn("absolute inset-0 flex items-center justify-center bg-black/40 transition-opacity duration-300", isHovered ? "opacity-0" : "opacity-100")}>
      ...
      <div className="absolute bottom-3 left-3 flex flex-wrap gap-1.5">
        ...
        {item.tags.length > 3 && (
          <span className="px-2 py-0.5 rounded-full bg-black/50 backdrop-blur-sm text-xs text-white/80">+{item.tags.length - 3}</span>
        )}
      </div>
  ...
  const renderMetadata = () => (
    showMetadata && (
      <div className="flex items-center gap-4 text-muted-foreground border-b border-border/30" style={{ padding: `${advancedSettings.headerPadding * 0.5}px ${advancedSettings.headerPadding}px`, fontSize: advancedSettings.metadataFontSize }}>
        <span className="flex items-center gap-1">
          <Calendar style={{ width: advancedSettings.metadataFontSize, height: advancedSettings.metadataFontSize }} />{formattedDateTime}
        </span>
        <span className="flex items-center gap-1">
          <HardDrive style={{ width: advancedSettings.metadataFontSize, height: advancedSettings.metadataFontSize }} />{formatFileSize(item.size)}
        </span>
        <span className="uppercase font-medium text-primary/80">{item.type === 'image' ? 'Photo' : 'Vidéo'}</span>
      </div>
    )
  );
  ...
  const renderActions = () => (
    showActions && (
      <div className="px-2 py-1 flex items-center justify-between">
        <Button variant="ghost" className="flex-1 gap-2 text-muted-foreground hover:text-primary hover:bg-primary/10" style={{ height: advancedSettings.actionButtonSize }} onClick={(e) => { e.stopPropagation(); onView(); }}>
        ...
        <Button variant="ghost" className={cn("flex-1 gap-2 transition-colors", isLiked ? "text-red-500 hover:text-red-600 hover:bg-red-500/10" : "text-muted-foreground hover:text-red-500 hover:bg-red-500/10")} style={{ height: advancedSettings.actionButtonSize }} onClick={handleLike}>
          <Heart className={cn(isLiked && "fill-current")} style={{ width: advancedSettings.actionIconSize, height: advancedSettings.actionIconSize }} />
          {showActionText && <span style={{ fontSize: advancedSettings.actionIconSize * 0.75 }}>{isLiked ? 'Favori' : "J'aime"}</span>}
        </Button>
